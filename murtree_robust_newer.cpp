...
vector<tree*> filter(int lowerBound, int upperBound, vector<tree*> trees) {
    vector<tree*> filteredTrees = {};
    //for(int j = 0; j < 3; j++)
...
tree_solutions merge(const vector<instance_t*> insts,const vector<instance_t*> globalInsts, int feature, tree_solutions left, tree_solutions right){
    ...
    int c = 0;
    int a = 0;
    for (tree* r : right.trees) {
        // cerr << "a" << endl;
        // cerr << a << endl;
        a++;
        for (tree* l : left.trees) {
            // cerr << "c" << endl;
            // cerr << c << endl;
            c++;
            // cerr << right.trees.size() << endl;
...
tree_solutions calculate_smallest_misclassification(const vector<instance_t*> insts, const vector<instance_t*> globalInsts, int depth, long long path[5], int upper_bound, const vector<int> feature_list) {
    ...
    if (mem.find(key) == mem.end()) {
        ...
        if (depth == 0) {
            ...
            tree_solutions candidates;  // Initialize pointer with valid object
            candidates.upperBound = upper_bound;
            candidates.lowerBound = lowest_bound_sofar;

            for (tree_solutions solution : viable_solutions) {
                
                candidates.trees = filter(lowest_bound_sofar, upper_bound, solution.trees);
...
//vector<instance_t*> insts2 {};
int main() {
    const string filename = "hypothyroid2.in";  // Replace with your .in file
    //insts2 = readInstances(filename);
